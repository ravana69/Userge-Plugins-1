...
# à´µà´¿à´µà´°à´£à´ à´à´à´¿à´àµà´àµà´®à´¾à´±àµà´±à´¿à´àµà´àµà´£àµà´àµ à´ªàµà´àµà´¨àµà´¨à´µàµ¼ à´àµà´°àµà´¡à´¿à´±àµà´±àµ à´µàµà´àµà´à´¾àµ½ à´¸à´¨àµà´¤àµà´·à´®àµ à´à´³àµà´³àµ..!
...
async def setup_google_photos(message: Message):
    ...
    if not creds:
        ...
        await message.edit("CREDS created. ððð")
    else:
        await message.edit("CREDS already created. ð")
...
async def upload_google_photos(message: Message):
    creds = await check_creds(message)
    if not creds:
        await message.edit_text("ð <code>gpsetup</code> first ð¡ðð", parse_mode="html")
        return
    path_ = ""
    ...
    await message.edit_text("uploaded to Google Photos, getting FILE URI ð¤ð¤")
